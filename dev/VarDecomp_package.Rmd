...
#'
#' @param brmsfit The output of a brms model. You can use VarDecomp::brms_model() to produce a brmsfit. 
#' @return Returns a plot with the highest probability density intervals of fixed effect estimates.
#' @export 
#' 

plot_intervals = function(brmsfit){

# Select fixed effects slope values
...
# Standardize the slopes for the standard deviation of each covariate (Z transformation)
    ...
    mutate(Treatment_m_Z = Treatment_m * sd(c(0,1)),
           Morph_m_Z = Morph_m * sd(c(0,1,2,3)),
           Sex_m_Z = Sex_m *sd(c(-0.5,0.5)),
           DaysAfterMolt_m_Z = DaysAfterMolt_m * sd(md$DaysAfterMoltC),
           Daytime_m_Z = Daytime_m * sd(md$DayTimeC),
           Temperature_m_Z = Temperature_m * sd(md$TemperatureC),
           JumpOrder_m_Z = JumpOrder_m * sd(md$JumpOrderC),
           Cohort_m_Z = Cohort_m * sd(md$CohortC),
           TreatmentMorph_m_Z = TreatmentMorph_m * 
                             sd(md$TreatmentC * md$ColorMorphC),
           
           Treatment_sd_Z = Treatment_sd * sd(c(0,1)),
           Morph_sd_Z = Morph_sd * sd(c(0,1,2,3)),
           Sex_sd_Z = Sex_sd *sd(c(-0.5,0.5)),
           DaysAfterMolt_sd_Z = DaysAfterMolt_sd * sd(md$DaysAfterMoltC),
           Daytime_sd_Z = Daytime_sd * sd(md$DayTimeC),
           Temperature_sd_Z = Temperature_sd * sd(md$TemperatureC),
           JumpOrder_sd_Z = JumpOrder_sd * sd(md$JumpOrderC),
           Cohort_sd_Z = Cohort_sd * sd(md$CohortC),
           TreatmentMorph_sd_Z = TreatmentMorph_sd * 
                             sd(md$TreatmentC * md$ColorMorphC)) %>% 
  select(ends_with("Z")) 
...
PSTempLong = gather(PSTemp,FixedEffect_Level, Slope) %>% 
  separate(col = FixedEffect_Level, 
           into = c("FixedEffect", "Level", "A"), 
           sep ="_", 
           remove = TRUE) %>% 
  select(!A) %>% 
mutate(FixedEffect = factor(FixedEffect, 
            levels = c("TreatmentMorph", "Cohort","JumpOrder",
                       "Temperature",  "Daytime", "DaysAfterMolt",
                       "Sex","Morph", "Treatment"), 
            labels = c("Treatment : Color Morph", "Cohort","Jump order",
                       "Temperature",  "Daytime", "Days since last molt",
                       "Sex","Color Morph", "Treatment"))) %>% 
  mutate(Level = recode(Level, "m" = "Average", "sd" = "Sigma")) %>% 
  filter(Level == level)


# Plot

ggplot(PSTempLong,aes(y = FixedEffect, x = Slope)) +  
  stat_halfeye(.width = c(.5, .95))+
  geom_vline(xintercept=0,color = "red")+
  labs(y = paste0(trait, " - ", level), x = "Fixed effects slopes")+
  theme_test()  
}
