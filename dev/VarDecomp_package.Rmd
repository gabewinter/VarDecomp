...
Data = Data %>%
  mutate(observationID = as.factor(row_number(Data)))
...
## Create a dataframe with the values used by the model for fixed effects
  ...
  select(-1)  #Removing the response variable
...
PS = PS %>% 
mutate(var_FixedEffects = rowSums(across(all_of(FixedEffectNames)), na.rm = TRUE))
...
if(!is.null(RandomSlope)){
  ...
  PS = PS %>% 
  mutate(!!paste0("var_", RandomEffect, "_", RandomSlope) := (get(paste0(RandomEffect, "_", RandomSlope))^2 *
...
if(Family == "gaussian"){
PS = PS %>% 
  mutate(residual = sigma^2)

} else {
  if(Family == "binomial"){

    PS = PS %>% 
      mutate(residual = observationID^2 + (pi^2/3))

    } else {
     if(Family== "poisson") {
       
      PS = PS %>% 
        mutate(residual =  observationID^2 + 
...
PS = PS %>% 
    mutate(total_pv = var_FixedEffects + 
...
for (i in FixedEffectNames){
        PS = PS %>% 
           mutate(!!paste0("R2_", i) := get(paste0(i))/total_pv)
}

PS = PS %>% 
mutate(R2_FixedEffects = rowSums(across(all_of(paste0("R2_", FixedEffectNames))), na.rm = TRUE))%>%          rename_with(~str_replace_all(., "_var_", "_"), contains("_var_"))

# calculate R2 for random slopes
    if(!is.null(RandomSlope)){
      
      PS = PS %>%
        mutate(!!paste0("R2_", RandomEffect, "_", RandomSlope) := 
...
PS = PS %>% 
  mutate(!!paste0("R2_",RandomEffect) := 
