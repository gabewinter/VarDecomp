...
if(length(FixedEffectName) > 0){
  ...
  FixedEffectData = Data %>%
      dplyr::select(any_of(FixedEffectName)) 
...
EstSummary = dplyr::left_join(EstSummary, HPDInt, by = "variable") %>% 
  dplyr::mutate(dplyr::across(tidyselect::where(is.numeric), \(x) round(x, 3))) %>% 
...
```{r test-model_summary, eval = FALSE}
...
```{r test-compare_slopes, eval = FALSE}
...
CharacterFE = prob::setdiff(FixedEffectName, PSFixedEffectName)
...
```{r test-plot_intervals, eval = FALSE}
#brms_model
...
PS = PS %>%
  dplyr::select(!RandomEffects) %>% 
  dplyr:select(!FixedEffects) %>% 
  tidyr::pivot_longer(cols = tidyselect::everything(), 
  names_to = "Variable",
  values_to = "R2") 
...
PS = PS %>%
  ...
  levels = c(prob::setdiff(PS$Variable, "residual"), "residual")))
